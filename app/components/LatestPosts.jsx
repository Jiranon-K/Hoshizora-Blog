...
import {
  ...
  LatestPostsItem,
  LatestPostsLink,
} from "./motion/LatestPostsAnimations";
          ...
          <div className="p-6 flex-grow flex flex-col">
            ...
            <div className="flex items-center mt-auto pt-4 border-t border-zinc-900">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <p className="text-xs font-light text-zinc-300">{author}</p>
                  <p className="text-[10px] text-zinc-600">{date}</p>
                </div>
              </div>

              <div className="ml-auto">
                <span className="text-xs text-zinc-500 uppercase tracking-widest group-hover:text-white transition-colors">
                  อ่านต่อ
                </span>
              </div>
            </div>
...
export default async function LatestPosts() {
      ...
      <div className="container mx-auto px-4 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-pulse">
          {[...Array(6)].map((_, index) => (
            <div
              key={index}
              className="bg-zinc-950 border border-zinc-900 h-96 flex flex-col"
            >
  ...
  return (
    <div className="container mx-auto px-4 py-16">
      <LatestPostsHeader className="flex justify-between items-end mb-12 border-b border-zinc-900 pb-4">
        <div>
      ...
      <LatestPostsGrid className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {posts.map((post) => (
          <PostCard
            key={post.id}
            title={post.title}
