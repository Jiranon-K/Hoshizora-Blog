        ...
        <div className="absolute bottom-0 left-0 right-0 p-8 z-20 flex flex-col justify-end h-full">
          ...
          <div className="flex items-center mt-auto pt-4 border-t border-white/10 w-full">
            <div className="w-6 h-6 rounded-full overflow-hidden border border-zinc-700 mr-2">
              <img
                src={getImageUrl(post.author_avatar || "/avatar/default.webp")}
                alt={post.author}
                className="w-full h-full object-cover"
              />
            </div>
            <span className="text-xs font-light text-zinc-300 uppercase tracking-wider">
              {post.author}
            </span>
  ...
  useEffect(() => {
    async function fetchFeaturedPosts() {
      try {
        ...
        if (
          settings.featured?.postIds &&
          settings.featured.postIds.length > 0
        ) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-pulse">
        ...
        <div className="grid grid-cols-1 gap-6">
          {[...Array(2)].map((_, index) => (
            <div
              key={index}
              className="h-60 bg-zinc-950 border border-zinc-900 p-6 flex flex-col justify-end"
            >
